...
def load_profits(data_dir, mev_counts):
    ...
    for mev_count in mev_counts:
        ...
        if os.path.exists(pbs_dir) and os.path.exists(pos_dir):
            ...
            pbs_profits = pbs_df[['creator_id', 'fee', 'type_of_user']]
            pos_profits = pos_df[['creator_id', 'fee', 'type_of_user']]
...
def plot_gini_coefficient(data_dir, mev_counts):
    ...
    for system in ['pbs', 'pos']:
        for mev_count in mev_counts:
            if mev_count in profits[system]:
                gini_coefficients[system][mev_count] = compute_gini(profits[system][mev_count]['fee'].values)
    ...
    sns.lineplot(x=x_pbs_new, y=y_pbs_new, marker='o', label='PBS', ax=ax, color=color_palette[0])
    sns.lineplot(x=x_pos_new, y=y_pos_new, marker='o', label='POS', ax=ax, color=color_palette[1])
...
def plot_profit_distribution(data_dir, mev_counts_to_plot):
    ...
    for i, mev_count in enumerate(mev_counts_to_plot):
        for system, color in zip(systems, color_palette):
            if mev_count in profits[system]:
                valid_profits = profits[system][mev_count]['fee'].values
                valid_profits = valid_profits[valid_profits >= 0]  # Ensure no negative values
                print(f"{system.upper()} MEV {mev_count} valid profits: {valid_profits}")
...
def plot_profit_distribution_violin(data_dir, mev_counts_to_plot):
    ...
    for i, mev_count in enumerate(mev_counts_to_plot):
        if mev_count in profits['pbs'] and mev_count in profits['pos']:
            ...
            sns.violinplot(data=combined_profits, x='type_of_user', y='log_fee', hue='type', split=True, inner='quart', ax=axes[i], palette=color_palette)
