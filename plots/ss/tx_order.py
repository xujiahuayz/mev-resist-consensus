...
def plot_heatmap(results, title, vmin, vmax, output_folder, x_label):
    ...
    plt.ylabel(r"MEV-Seeking Users $\tau_{U_i} = \mathtt{attack}$ (%)", fontsize=30)
...
if __name__ == "__main__":
    ...
    plot_heatmap(pos_results, "Inversion Counts for PoS", vmin, vmax, output_folder, r"MEV-Seeking Validators $\tau_{V_i} = \mathtt{attack}$ (%)")

    # Plot heatmap of results for PBS with "Builders" as x-axis label
    plot_heatmap(pbs_results, "Inversion Counts for PBS", vmin, vmax, output_folder, r"MEV-Seeking Builders $\tau_{B_i} = \mathtt{attack}$ (%)")
