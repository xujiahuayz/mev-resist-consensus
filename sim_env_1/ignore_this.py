...
class Builder:
    ...
    def create_block(self, transactions):
        ...
        self.bid = random.uniform(
            0, sum(transaction[1] for transaction in transactions)
        )
...
def simulate_game(num_builders, num_transactions, num_blocks):
    ...
    for _ in range(num_blocks):
        ...
        transactions = [
            (f"Transaction{i}", random.uniform(1, 10)) for i in range(num_transactions)
        ]
        ...
        if winner_id is not None:
            ...
            print(
                f"Block { _ + 1 }: Winner - Builder {winner.id}, Winning Bid - {winning_bid}"
            )
            # Reward the winner and proposer
            winner_reward = (
                sum(transaction[1] for transaction in winner.block) - winning_bid
            )
            proposer_reward = winning_bid
            print(
                f"Builder {winner.id} Reward: {winner_reward}, Proposer Reward: {proposer_reward}"
            )
        else:
